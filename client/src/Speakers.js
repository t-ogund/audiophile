...
function Speakers() {
    ...
    let [ speakerData, setSpeakerData ] = useState([]);

    useEffect(() => {
        fetch('/headphone-data')
          .then(res => res.json())
          .then(data => setSpeakerData(data.data.filter(item => item.category === "speakers")))
      }, [])

      const speakerComponents = speakerData.map((item, index) => {
