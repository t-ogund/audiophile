...
function Headphones() {
    let [ headphoneData, setHeadphoneData ] = useState([]);

    useEffect(() => {
        fetch('/headphone-data')
          .then(res => res.json())
          .then(data => setHeadphoneData(data.data.filter(item => item.category === "headphones")))
      }, [])

      const headphoneComponents = headphoneData.map((item, index) => {
        return <HeadphoneComponent
        mapKey={index} 
        name={ item.name } 
